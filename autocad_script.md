...
　モノづくり系のサークルで活動しているときに「同じような図面描くの面倒だなぁ」「大量の座標データを流し込めないかなぁ」と思い調べたのでメモ。

　学生は無料、ということでAutodesk AutoCADを使っています。

# 概要
　方法はいたって単純。AutoCADのコマンドを書き溜めてコピー＆コマンドライン↓にペーストするだけ。

![image](resources/autocad1.png)

　クリップボードにコピーするだけなので、エディタはメモ帳・Atom・Sublime etc…なんでもOKです。

　※本来、AutoCADにはコマンドスクリプトという機能(`.scr`ファイルの中のコマンドを実行する)があるようなのですが、コマンドラインに直打ちするときとは微妙に挙動が異なるようです(Optionのせいかもしれない)。調べた限り作図に応用している例も見当たらなかったので今回は諦め。

### 動作確認環境
　一応、自分の開発環境です。

* Windows10 Home Premium / OS X Yosemite
* AutoCAD 2017 Student Version / AutoCAD 2014 FOR MAC
* Atom
* Excel 2016

# Scriptを書くときの注意点

1. `<Enter>`と`<Space>`はどちらも適用操作と見なされる。
...
　実際にコマンドを実行してみながらエディタに書き溜めていくことをお勧めします。実行時に何かのコマンドが起動しているとうまく動かないので`<ESC><ESC>`と叩いてからペーストすると安心です。

# 実際に描いてみる

　それでは基本図形から描いてみましょう。下のコード部分をコピーしてコマンドラインに張り付ければ動くようになっていますのでよければ実際に試してみてください。

　ちなみに、デフォルトの設定では「`--`」から始まるコマンドはないので、コメント行もそのままコピーしてしまって大丈夫です。
...
```draw_circles.txt
--(100,100)を中心にした半径100の円
circle 100,100 100
--(0,0)を中心にした直径100の円
circle 0,0 d 100
--3点(0,0)(100,100)(100,0)を通る円
circle 3p 0,0 100,100 100,0
...
--draw_spline.txtの続き
--(200,0)周辺を通っているスプラインを選択して左に100移動させる(base:(0,0)->(-100,0))
move 200,0  0,0 -100,0
--(-10,-10)-(410,210)の窓に収まっているスプラインを選択して(-100,210)ずらしたところにコピーする
copy w -10,-10 410,210  0,0 -100,210 e
...
circle 0,0 35
--(0,250)間隔で9個Copy
copy crossing 0,0 1000,0  0,0 0,250 0,500 0,750 0,1000 0,1250 0,1500 0,1750 0,2000 0,2250 0,2500 e
--桁穴を避けるように外側だけ選択し桁穴を中心にそれぞれの縮小率で縮小
scale crossing 100,250 1000,250  0,250 0.98
scale crossing 100,500 1000,500  0,500 0.96
scale crossing 100,750 1000,750  0,750 0.94
scale crossing 100,1000 1000,1000  0,1000 0.92
scale crossing 100,1250 1000,1250  0,1250 0.9
scale crossing 100,1500 1000,1500  0,1500 0.88
scale crossing 100,1750 1000,1750  0,1750 0.86
scale crossing 100,2000 1000,2000  0,2000 0.84
scale crossing 100,2250 1000,2250  0,2250 0.82
scale crossing 100,2500 1000,2500  0,2500 0.8
